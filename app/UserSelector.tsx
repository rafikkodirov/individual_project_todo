...
const UserSelector: React.FC<UserSelectorProps> = ({ visible, onClose, onSelectUser }) => {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = cachedUsers.filter(users => users.nickname.toLowerCase().includes(query.toLowerCase()));
    setFilteredGroups(filtered);
  };
  const showSearch = cachedUsers.length > 7
const ITEM_HEIGHT = 50
  const displayedUsers = searchQuery.trim() ? filteredGroups : cachedUsers;
  return (
