...
import { useTaskActions } from '../functions/taskActions';
import { useLocalSearchParams } from 'expo-router';
const styles = Platform.OS === 'android'
  ? require('../../../styles/styles.android').default
  : require('../../../styles/styles.android').default;
const DefUserTasks: React.FC = () => {
  const [confirmationDialogVisible, setConfirmationDialogVisible] = useState<string>('');

  const { concatenateTasks, userData,selectedGroupId} = useDataContext();
  ...
  console.log(userId,'uniqueTasks UserID');
  const definedTask = useMemo(() => {
    return uniqueTasks.filter(
      (task) =>
        task.performerId === userId && task.groupId === selectedGroupId
    );
  }, [uniqueTasks, userId, selectedGroupId]);
